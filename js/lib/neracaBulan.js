define(['jquery', 'main', 'axios', 'crud', 'helper', 'select2', 'terbilang'], function($, main, axios, crud, helper, select2, angkaTerbilang) {
  let obj2 = {
    neraca: async function(aw){
      ...
      let selectN = helper.yearChoice(10, `tahun-action script-data="neracaBulan.rugilaba()" style="width:200px;"`, 'select2');

      let bulanPilih = helper.monthChoice(`bulan-action script-data="neracaBulan.rugilaba()" style="width:200px;"`, 'select2');
      ...
      let type = await helper.decryptG(helper.localGet('type_akun')).forEach(function(res, i) {
        ...
        if (aw === "Neraca") {
          ...
          let laba = Number(helper.sesiGet('labarugi')) - totalNominalKredit;
          ...
          <td style="border-right: none;" width="20px">Rp</td>
          <td style="border-left: none; text-align: right;" width="150px">${helper.formatRupiah(Math.ceil(laba))}</td>
          </tr>
          </tbody>
          <table>
          </div>
          ...
          helper.sesiNew('labarugi', Number(totalNominalDebet) - Number(totalNominalKredit) );
        }
