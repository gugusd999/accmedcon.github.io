define(['jquery', 'axios', 'form', 'helper', 'live', 'select2', 'jQueryInputmask'], function($, axios, form, helper,  live, select2){
  ...
  crud.view = async function(a = null){
    ...
    html +=  `<button table-name="${a}" go-crud-edit-3 class="btn btn-primary btn-sm mb-3 mt-3">Ubah</button>`;
    ...
    document.querySelector(`body #include-master-data`)
    .innerHTML = html;
  ...
  crud.edit = async function(a = null){
    ...
    html +=  `<button table-name="${a}" go-back-crud-view-3 class="btn btn-default btn-sm mb-3 mt-3 mr-3">Kembali</button>`;
    html +=  `<button table-name="${a}" data-send="${dataSend}" tambah-data-baru-crud-3 class="btn mt-3 btn-primary btn-sm mb-3">Tambah</button>`;
    ...
    dataIsiTable.forEach((item) => {
      ...
      html +=  `<td width="80px"><button table-name="${crudData.tablename}" data-key="${crudData.key}" crud-data-hapus-3 data-id="${eval(`item.${crudData.key}`)}" class="btn btn-danger">Hapus</button></td>`;
