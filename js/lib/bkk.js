  ...
  let obj2 = {
    bkk: async function(){
      ...
      jurnal.umum = async function(){
        ...
        let data2 = await helper.decryptG(helper.localGet('transaksi_pembelian_barang')).filter(function(item){
          let fill = helper.sesiGet('tahun')+'-'+helper.sesiGet('bulan');
          console.log(item.transaksi);
          if (item.transaksi != undefined) {
            let fill2 = helper.potongText(item.tanggal_transaksi, 0, 7);
            if (fill == fill2) {
              return item;
            }
        ...
        <tr>
          <td>${number}</td>
          <td>${dataB}-${item}</td>
          <td><button style="text-align:center;" data-no-transaksi="${item}" class="btn-custome" bkk-print script-data="bkk.view('bkk-view')" >Lihat</button></td>
        </tr>
            `;
            // console.log();
            number++;
    ...
    bkkView: function(){
      ...
      data.forEach((item) => {
          ...
          let {sekarang} = helper.tanggal(item.tanggal_nota);
